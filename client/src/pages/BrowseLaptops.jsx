...
export default function BrowseLaptops()
{
    ...
    const fetchAllLaptops = async() =>
    {
        try
        {
            ...
            if(response.ok)
            {
                ...
                //console.log('Fetched laptops:', fetchedLaptops);
    ...
    (
        (schedule, index) =>
            <option key={index} value={schedule.course}> {schedule.course} - {schedule.section} </option>
    );
        ...
        <>
            ...
            { showForm ? 
                <form className="borrowform" onSubmit={handleSubmit}>
                    ...
                    <select id="schedule" name="schedule" value={formInput.scheduleCourse} onChange={(e) => setFormInput((prev) => ({...prev, scheduleCourse: e.target.value}))} required>
                        <option> </option>
                        {showSchedulesAsOptions}
                    </select>
                    <br/>
                    <label htmlFor="reason"> Reason </label>
                    <textarea id="reason" name="reason" rows="5" value={formInput.reason} onChange={(e) => setFormInput((prev) => ({...prev, reason: e.target.value}))} required/>
                    <div>
